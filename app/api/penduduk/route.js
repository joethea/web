...
export async function GET(req) {
  try {
    ...
    if (distinctField) {
      if (distinctField === "bantuan_sosial") {
        const { data, error } = await supabase
          .from("penduduk_sistem")
          .select("kolum_bantuan")
          .neq("kolum_bantuan", null)
          .not("kolum_bantuan", "eq", "")
          .order("kolum_bantuan", { ascending: true });

        if (error) throw error;

        const distinctValues = [
          ...new Set((data || []).map((item) => item.kolum_bantuan)),
        ];
    ...
    const bantuan_sosial = url.searchParams.get("bantuan_sosial") || "";
    ...
    // ✅ filter bantuan_sosial multi-value
    if (bantuan_sosial) {
      const bantuanArray = bantuan_sosial.split(",").map((b) => b.trim());
      if (bantuanArray.length > 0) {
        baseQuery = baseQuery.contains("bantuan_sosial", bantuanArray);
      }
    }
...
export async function POST(req) {
  try {
    ...
    const { error } = await supabase.from("penduduk").insert([{
      ...
      hubungan_keluarga, yatim_piatu, miskin_sangat,
      kategori_mengaji, lokasi_mengaji, status_rumah,
      bantuan_sosial: Array.isArray(bantuan_sosial) ? bantuan_sosial : (bantuan_sosial ? [bantuan_sosial] : [])
    }]);
