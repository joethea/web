...
export default function PendudukPage() {
  ...
  const handleFileChange = (file) => {
    if (file && !importInProgress.current) {
      ...
      importExcelFile(file, ambilDataPenduduk, setImportLoading).finally(() => {
  ...
  const warnaBadge = (miskinSangat) => {
    ...
    return "bg-slate-300 text-gray-900";
  };

  const BadgeEkonomi = ({ status }) => {
    if (!status) return null;

    let warna = "bg-gray-400 text-white";
    ...
    return <span className={`px-3 py-1 rounded-full text-xl font-semibold ${warna}`}>{status}</span>;
  };
  ...
  function MultiSelectBantuan({ options, selected, onChange }) {
    ...
    return (
      <div ref={wrapperRef} className="relative w-full">
        ...
        >
          <span>{selected.length > 0 ? selected.join(", ") : "Pilih Bantuan Sosial"}</span>
          <svg
  ...
  const hitungHantu = async () => {
    try {
      const { data, error } = await supabase.from("penduduk").select("id, req_delete");

      if (error) throw error;

      const count = data.filter((p) => {
        const val = (p.req_delete ?? "").toString().toLowerCase().trim();
        return val === "true" || val === "1" || val === "y" || val === "yes" || val === "t";
      }).length;
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "penduduk" }, () => {
  ...
  const ambilLokasiMengaji = useCallback(async (kategori) => {
    ...
    try {
      const res = await fetch(`/api/penduduk?distinct=lokasi_mengaji&kategori=${kategoriParam}`);
      const hasil = await res.json();
  ...
  const applyFilters = () => {
    ...
    ambilDataPenduduk(
      ...
      draftFilterDusun,
      draftFilterStatus,
      draftFilterPekerjaan,
      draftFilterPendidikan,
      draftFilterJK,
      draftFilterHubunganKeluarga,
      draftUmurMin,
      draftUmurMax,
      true,
      draftFilterYatim,
  ...
  const adaFilterAktif = useMemo(() => {
    return Boolean(
      cariNama ||
        ...
        filterLokasiMengaji ||
        filterUsia ||
        (Array.isArray(filterBantuan) && filterBantuan.length > 0)
    );
  }, [
    cariNama,
    filterKK,
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      ...
      if (filterEl && !filterEl.contains(e.target) && filterBtn && !filterBtn.contains(e.target)) {
        setIsFilterGroupVisible(false);
      }
    };
  ...
  const [modeInfo, setModeInfo] = useState("total");
  ...
  const ambilDataPenduduk = useCallback(
    async (
      page = 1,
      modeEkspor = false,
  ...
  const ambilSemuaData = useCallback(async () => {
    try {
      ...
      setJumlahTotal(hasil.data.length);
      return hasil.data;
    } catch (err) {
  ...
  const isiDropdownsOtomatis = useCallback(async () => {
    ...
    const statusKeluargaUnik = [...new Set(data.map((p) => p.status_keluarga).filter(Boolean))];
    const statusYatimUnik = [...new Set(data.map((p) => p.status_yatim).filter(Boolean))];
    const kategoriMiskinUnik = [...new Set(data.map((p) => p.kategori_miskin).filter(Boolean))];
    ...
    const skSelect = document.getElementById("filterStatusKeluarga");
    const sySelect = document.getElementById("filterStatusYatim");
    const kmSelect = document.getElementById("filterKategoriMiskin");
    ...
    if (kerjaSelect) {
      try {
        ...
        const pekerjaanList =
          typeof pekerjaanUnik !== "undefined" && Array.isArray(pekerjaanUnik) && pekerjaanUnik.length
            ? pekerjaanUnik
        ...
        kerjaSelect.style.color =
          typeof draftFilterPekerjaan !== "undefined" && draftFilterPekerjaan ? "#1f2937" : "#9CA3AF";
    ...
    if (eSelect) {
      try {
        const ekonomiList = ["Sangat Miskin", "Miskin", "Kurang Mampu", "Mampu", "Kaya", "Sangat Kaya"];
        eSelect.innerHTML = "";
  ...
  if (kmSelect) {
    try {
      ...
      kmSelect.addEventListener("change", (e) => {
        if (e.target.value === "") {
          kmSelect.style.color = "#9CA3AF";
        } else {
          kmSelect.style.color = "#1f2937";
        }
      });
    } catch (err) {
  ...
  const tampilkanAnggotaKeluarga = useCallback(
    async (kk, nikTerpilih) => {
      ...
      try {
        ...
        const data = Array.isArray(hasil) ? hasil : Array.isArray(hasil?.data) ? hasil.data : [];
        ...
        if (nikTerpilih) {
          const anggotaTerpilih = sortedKeluarga.find((a) => String(a?.nik) === String(nikTerpilih));
          if (anggotaTerpilih) {
            setDetailAnggota(anggotaTerpilih);
          }
        } else if (detailAnggota) {
          const masihAda = sortedKeluarga.find((a) => String(a.nik) === String(detailAnggota.nik));
          if (masihAda) {
  ...
  const handleEdit = useCallback(
        ...
        <table class="w-full text-sm text-left border border-gray-200">
          <tbody class="divide-y divide-gray-100">
            <tr>
              ...
              <td><input id="kode_desa" class="w-full p-2 border rounded bg-gray-100 cursor-not-allowed" value="${
                data.kode_desa
              }" readonly /></td>
            </tr>
            <tr><th class="bg-gray-50 px-4 py-2 text-gray-600">No. KK</th><td><input id="kk" class="w-full p-2 border rounded" value="${
              data.kk
            }" /></td></tr>
            <tr><th class="bg-gray-50 px-4 py-2 text-gray-600">NIK</th><td><input id="nik" class="w-full p-2 border rounded" value="${
              data.nik
            }" /></td></tr>
            <tr><th class="bg-gray-50 px-4 py-2 text-gray-600">Nama</th><td><input id="nama" class="w-full p-2 border rounded" value="${
              data.nama
            }" /></td></tr>
            <tr>
              ...
              <td>
                <div class="flex gap-2">
                  <input id="tempat_lahir" class="w-1/2 p-2 border rounded" value="${
                    data.tempat_lahir
                  }" />
                  <input id="tanggal_lahir" type="text" class="w-1/2 p-2 border rounded" value="${formatTanggalIndo(
                    data.tanggal_lahir
                  )}" />
                </div>
              </td>
              ...
              <td>
                <select id="pekerjaan" class="w-full p-2 border rounded">
                  ...
                  <option value="Buruh Tani/Perkebunan" ${
                    data.pekerjaan === "Buruh Tani/Perkebunan" ? "selected" : ""
                  }>Buruh Tani/Perkebunan</option>
                  <option value="Guru" ${data.pekerjaan === "Guru" ? "selected" : ""}>Guru</option>
                  <option value="Karyawan Honorer" ${
                    data.pekerjaan === "Karyawan Honorer" ? "selected" : ""
                  }>Karyawan Honorer</option>
                  <option value="Karyawan Swasta" ${
                    data.pekerjaan === "Karyawan Swasta" ? "selected" : ""
                  ...
                  <option value="Petani/Pekebun" ${
                    data.pekerjaan === "Petani/Pekebun" ? "selected" : ""
                  ...
                  <option value="Tukang Las/Pandai Besi" ${
                    data.pekerjaan === "Tukang Las/Pandai Besi" ? "selected" : ""
                  }>Tukang Las/Pandai Besi</option>
                  <option value="Ustadz/Mubaligh" ${
                    data.pekerjaan === "Ustadz/Mubaligh" ? "selected" : ""
            ...
            <tr>
              ...
              <td>
                <select id="pendidikan" class="w-full p-2 border rounded mb-2">
                  ...
                  <option value="Tidak/Blm Sekolah" ${
                    data.pendidikan === "Tidak/Blm Sekolah" ? "selected" : ""
                  }>Tidak/Blm Sekolah</option>
                  <option value="Blm Tmt SD/Sederajat" ${
                    data.pendidikan === "Blm Tmt SD/Sederajat" ? "selected" : ""
                  }>Blm Tmt SD/Sederajat</option>
                  ...
                  <option value="Diploma IV/Strata 1" ${
                    data.pendidikan === "Diploma IV/Strata 1" ? "selected" : ""
                  }>Diploma IV/Strata 1</option>
              ...
              <td>
                <select id="hubungan_keluarga" class="w-full p-2 border rounded">
                  ...
                  <option value="Kepala Keluarga" ${
                    data.hubungan_keluarga === "Kepala Keluarga" ? "selected" : ""
              ...
              <td>
                <select id="status_rumah_edit" class="w-full p-2 border rounded">
                  ...
                  <option value="Rumah Sendiri" ${
                    data.status_rumah === "Rumah Sendiri" ? "selected" : ""
            ...
            <tr><th class="bg-gray-50 px-4 py-2 text-gray-600">Desa</th><td><input id="desa_edit" class="w-full p-2 border rounded bg-gray-100 cursor-not-allowed" value="${
              data.desa
            }" disabled /></td></tr>
            <tr>
              ...
              <td>
                <select id="miskin_sangat" class="w-full p-2 border rounded">
                  ...
                  <option value="Sangat Miskin" ${
                    data.miskin_sangat === "Sangat Miskin" ? "selected" : ""
                  }>Sangat Miskin</option>
                  <option value="Miskin" ${data.miskin_sangat === "Miskin" ? "selected" : ""}>Miskin</option>
                  <option value="Kurang Mampu" ${
                    data.miskin_sangat === "Kurang Mampu" ? "selected" : ""
                  }>Kurang Mampu</option>
                  ...
                  <option value="Sangat Kaya" ${
                    data.miskin_sangat === "Sangat Kaya" ? "selected" : ""
                  }>Sangat Kaya</option>
                </select>
              </td>
            </tr>
            ...
            <tr>
              ...
              <td>
                <select id="kategori_mengaji_edit" class="w-full p-2 border rounded">
                  <option value="">--Tidak Ada--</option>
                  <option value="Ngaji Lokal" ${
                    data.kategori_mengaji === "Ngaji Lokal" ? "selected" : ""
                  }>Ngaji Lokal</option>
                  <option value="Ngaji Luar" ${
                    data.kategori_mengaji === "Ngaji Luar" ? "selected" : ""
                  }>Ngaji Luar</option>
                  <option value="Guru Ngaji" ${
                    data.kategori_mengaji === "Guru Ngaji" ? "selected" : ""
                  }>Guru Ngaji</option>
                </select>
              </td>
            </tr>
    ...
    `,
        ...
        didOpen: async () => {
          ...
          const kategoriAwal =
            data.kategori_mengaji === "Ngaji Lokal"
              ? "lokal"
              : data.kategori_mengaji === "Ngaji Luar"
              ? "luar"
              : "guru";
          ...
          try {
            ...
            if (container) {
              ...
              opsiBantuan.forEach((opt) => {
                ...
                label.innerHTML = `<input type="checkbox" id="${id}" value="${opt}" ${
                  checked ? "checked" : ""
                } style="margin-right:8px" /> <span style="white-space:nowrap">${opt}</span>`;
...
`,
        ...
        preConfirm: () => {
          const popup = Swal.getPopup();
          const checked = Array.from(popup.querySelectorAll(".swal-column:checked")).map((el) => el.value);
      ...
      const selectedKeys = pilihan.value;
        ...
        sortedData.forEach((row) => {
          ...
          selectedKeys.forEach((k) => {
            if (k === "tanggal_lahir") {
              ...
              newRow[k] = Array.isArray(row.bantuan_sosial) ? row.bantuan_sosial.join(", ") : row.bantuan_sosial || "";
            } else {
              newRow[k] = row[k] ?? "";
            }
          });
      ...
      <div class="">
        <table class="w-full text-sm text-left border border-gray-200">
          <tbody class="divide-y divide-gray-100">
            <tr>
              <th class="bg-gray-50 px-4 py-2 text-gray-600 w-40">Kode Desa</th>
              <td><input id="kode_desa" class="w-full p-2 border rounded bg-gray-100 cursor-not-allowed" placeholder="Kode Desa" value="${
                defaultValues.kode_desa || "2036"
              }" readonly/></td>
            </tr>
            <tr>
              <th class="bg-gray-50 px-4 py-2 text-gray-600">No. KK</th>
              <td><input id="kk" class="w-full p-2 border rounded" placeholder="Nomor KK" value="${
                defaultValues.kk || ""
              }" /></td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td><input id="desa" class="w-full p-2 border rounded bg-gray-100 cursor-not-allowed" placeholder="Nama Desa" value="${
                defaultValues.desa || "Bugak Krueng"
              }" disabled/></td>
            </tr>
            <tr>
              <th class="bg-gray-50 px-4 py-2 text-gray-600">Status Yatim</th>
            ...
            <tr>
              ...
              <td>
                <select id="miskin_sangat" class="w-full p-2 border rounded">
                  <option value="">--Tidak Ada--</option>
                  <option value="Sangat Miskin" ${
                    defaultValues.miskin_sangat === "Sangat Miskin" ? "selected" : ""
                  }>Sangat Miskin</option>
                  <option value="Miskin" ${defaultValues.miskin_sangat === "Miskin" ? "selected" : ""}>Miskin</option>
                  <option value="Kurang Mampu" ${
                    defaultValues.miskin_sangat === "Kurang Mampu" ? "selected" : ""
                  }>Kurang Mampu</option>
                  ...
                  <option value="Sangat Kaya" ${
                    defaultValues.miskin_sangat === "Sangat Kaya" ? "selected" : ""
                  }>Sangat Kaya</option>
                </select>
              </td>
            </tr>
    ...
    const createBtn = (label, page, disabled = false, active = false) => (
      <button
        ...
        className={`px-3 py-1 rounded border text-sm ${
          active ? "bg-blue-600 text-white" : "bg-white text-gray-800 hover:bg-blue-100"
  ...
  return (
    <div className="max-w-full mx-auto relative">
      ...
      <div className="bg-white border-b-4 border-green-600 shadow-sm rounded-lg p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4 sticky top-0 z-40 relative">
        ...
        <h2 className="text-xl font-bold text-gray-700 flex items-center gap-2">
          <img src="/bireuen.png" alt="Data Penduduk" className="w-8 h-8 object-contain" />
          BGK KRUENG
        </h2>
    ...
    ${
        ...
        : "border-gray-200 opacity-40 cursor-not-allowed"
          ...
          >
            <svg
              ...
              className="w-6 h-6"
            >
              <path strokeLinecap="round" strokeLinejoin="round" d="M3 4h18l-6.5 8v6l-5 3v-9L3 4z" />
              <path
                d="M15.5 2.5l6 6m0-6l-6 6"
                stroke={adaFilterAktif ? "red" : "#9CA3AF"}
                strokeWidth="2.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              />
              ...
              >
                ...
                {isExportMenuOpen && (
                  <div className="absolute left-0 mt-2 w-48 bg-white text-gray-700 border border-gray-200 rounded shadow-lg z-50">
                    ...
                    <div>
                      <button
                        className="block w-full px-4 py-2 text-left hover:bg-yellow-50 text-yellow-700 cursor-pointer"
                        onClick={openFileInputPopup}
                      >
                        Import Excel
                      </button>
                    </div>
      ...
      <div className="px-3 py-1 bg-gray-100 text-gray-600 rounded-full text-sm font-medium shadow-sm">
        <InfoFilter jumlah={modeInfo === "filter" ? jumlahFilter : jumlahTotal} mode={modeInfo} />
      </div>

      {/* Overlay gelap */}
      {isFilterGroupVisible && (
        <div onClick={() => setIsFilterGroupVisible(false)} className="fixed inset-0 bg-black bg-opacity-20 z-40"></div>
      )}
      ...
      <div
        ...
        className={`absolute left-0 right-0 top-full mt-1 z-50 bg-gradient-to-br from-white via-gray-50 to-gray-100 border border-gray-200 rounded-xl shadow-2xl p-6
              ...
              ${isFilterGroupVisible ? "opacity-100 scale-100 visible" : "opacity-0 scale-90 invisible"}`}
      >
        {/* Cari Nama */}
        <input
          id="cariNama"
        ...
        <select
          id="filterDusun"
          className={`border border-gray-300 p-2 rounded w-full ${
            draftFilterDusun ? "text-gray-800" : "text-gray-400"
          }`}
        ...
        <select
          ...
          className={`border border-gray-300 p-2 rounded w-full ${
            draftFilterStatus ? "text-gray-800" : "text-gray-400"
          }`}
        >
        ...
        <select
          ...
          className={`border border-gray-300 p-2 rounded w-full ${
            draftFilterJK ? "text-gray-800" : "text-gray-400"
          }`}
        >
          <option value="">Jenis Kelamin</option>
        ...
        <select
          ...
          className={`border border-gray-300 p-2 rounded w-full ${
            draftFilterPekerjaan ? "text-gray-800" : "text-gray-400"
          }`}
        >
        ...
        >
          ...
          <option value="Sangat Miskin" style={{ color: "#1f2937" }}>
            Sangat Miskin
          </option>
          <option value="Miskin" style={{ color: "#1f2937" }}>
            Miskin
          </option>
          <option value="Kurang Mampu" style={{ color: "#1f2937" }}>
            Kurang Mampu
          </option>
          <option value="Mampu" style={{ color: "#1f2937" }}>
            Mampu
          </option>
          <option value="Kaya" style={{ color: "#1f2937" }}>
            Kaya
          </option>
          <option value="Sangat Kaya" style={{ color: "#1f2937" }}>
            Sangat Kaya
          </option>
        </select>
        ...
        >
          ...
          <option value="Ngaji Lokal" style={{ color: "#1f2937" }}>
            Ngaji Lokal
          </option>
          <option value="Ngaji Luar" style={{ color: "#1f2937" }}>
            Ngaji Luar
          </option>
          <option value="Guru Ngaji" style={{ color: "#1f2937" }}>
            Guru Ngaji
          </option>
        </select>
        ...
        >
          ...
          <option value="anak" style={{ color: "#1f2937" }}>
            Anak-anak
          </option>
          <option value="remaja" style={{ color: "#1f2937" }}>
            Remaja
          </option>
          <option value="dewasa" style={{ color: "#1f2937" }}>
            Dewasa
          </option>
          <option value="lansia" style={{ color: "#1f2937" }}>
            Lansia
          </option>
          <option value="produktif" style={{ color: "#1f2937" }}>
            Produktif
          </option>
          <option value="non-produktif" style={{ color: "#1f2937" }}>
            Non-Produktif
          </option>
        </select>
        ...
        <div className="mb-3">
          <MultiSelectBantuan options={opsiBantuan} selected={draftFilterBantuan} onChange={setDraftFilterBantuan} />
        </div>
        ...
        <div className="flex gap-2 col-span-full sm:col-span-2">
          ...
          <button
            ...
            onClick={() => {
              ...
              ambilDataPenduduk(1, false, "", "", "", "", "", "", "", "", "", "", false, "", "", "", "", "");
            }}
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        ...
        <div className="flex-grow overflow-x-auto bg-white rounded shadow">
              ...
              {loading ? (
                <tr>
                  <td colSpan="6" className="p-6 text-center">
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      <span className="text-gray-600 font-medium animate-pulse">Memuat data...</span>
                    </div>
                  </td>
                </tr>
              ) : penduduk.length === 0 ? (
                ...
                penduduk.map((p, i) => (
                  <tr
                    ...
                    data-nik={p.nik || ""}
                    onClick={() => {
                      setSelectedNik(String(p.nik));
                      tampilkanAnggotaKeluarga(p.kk, p.nik);
                    }}
                    className={`${
                      (p.req_delete || "").toLowerCase() === "true"
                  ...
                  >
                    ...
                    <td className="p-3 flex gap-3 justify-center">
                      <button
                        ...
                        className="flex items-center gap-0 bg-yellow-500 hover:bg-yellow-600 text-white px-1 py-1 rounded text-xs sm:text-sm transition"
                      >
          ...
          {/* Pagination */}
          <div className="mt-4 flex justify-center gap-2 ml-2 mb-4">{renderPagination()}</div>
        </div>
        ...
        <div className="w-full lg:w-1/3 flex flex-col gap-4">
          {(() => {
            ...
            let warnaHeader = "bg-slate-300 text-gray-900";
            if (kepala) {
            ...
            return (
              <div className="bg-white rounded shadow">
                {/* Header */}
                <div className="flex justify-between items-center px-4 py-3 rounded-t ${warnaHeader}">
                    ...
                    {kepala ? (
                      kategori ? (
                        ...
                        <>
                          ...
                          <span className="ml-2 px-3 py-1 rounded-full text-xs font-semibold bg-gray-300 text-black"></span>
                        </>
                      )
                    ) : (
                      <>
                ...
                <div className="px-3 py-4">
                  <ul id="listKeluarga" className="text-sm text-gray-700 min-h-[4rem] max-h-64 overflow-y-auto divide-y">
                    {anggotaKeluarga.length === 0 ? (
                      <li className="text-gray-400 px-2 py-1">Tidak ada anggota keluarga</li>
                    ) : (
                        ...
                        return (
                          ...
                          >
                            ...
                            <span
                              ...
                              className={`w-4 h-4 rounded-full inline-block ${warnaBadge(anggota.miskin_sangat)}`}
                            ></span>
                          </li>
                        );
                      })
              ...
              <div className="flex flex-wrap gap-2">
                {(() => {
                  const kk = anggotaKeluarga?.find(
                    (a) => a.hubungan_keluarga && a.hubungan_keluarga.toLowerCase() === "kepala keluarga"
                  );
                  ...
                  return bantuanList.length > 0 ? (
                    ...
                    <span className="text-gray-400 italic text-sm">Tidak ada data bantuan</span>
                  );
                })()}
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-md border border-gray-200">
                    ...
                    >
                      ...
                      >
                        <path
                          strokeLinecap="round"
                          strokeLinejoin="round"
                          d="M16.862 4.487l1.651 1.65a2 2 0 010 2.829l-9.193 9.192a4 4 0 01-1.632.96l-3.234.97.97-3.234a4 4 0 01.96-1.632l9.192-9.193a2 2 0 012.829 0z"
                        />
                      </svg>
                      <span>Edit</span>
                    </button>
            ...
            <div id="detailAnggotaKeluarga" className="text-sm text-gray-700">
              {detailAnggota ? (
                <table className="w-full border-collapse" style={{ marginTop: "10px", marginBottom: "15px" }}>
                  <tbody>
                    {[
                      ...
                      [
                        "Jenis Kelamin",
                        detailAnggota.jenis_kelamin === "L"
                          ? "Laki-laki"
                          : detailAnggota.jenis_kelamin === "P"
                          ? "Perempuan"
                          : "-",
                      ],
                      [
                        "Status Pernikahan",
                        detailAnggota.status === "S"
                          ? "Sudah Kawin"
                          : detailAnggota.status === "B"
                          ? "Belum Kawin"
                          : detailAnggota.status === "P"
                          ? "Pernah Kawin"
                          : "-",
                      ],
                      ...
                      <tr key={i} className="border-b last:border-none">
                        ...
                        <td
                          className={`px-3 py-1 ${
                            label === "Nama" ? "font-bold text-blue-700 text-sm" : "font-medium text-gray-800"
                          }`}
                          style={{ width: "63%" }}
                        >
                          {value || "-"}
                ...
                <p className="italic text-gray-500 p-4">Pilih anggota keluarga untuk melihat detail.</p>
              )}
            </div>
          </div>
        </div>
...
function getAgeDecimalOne(tanggalLahir) {
  ...
  return Number((tahun + bulan / 12).toFixed(1));
}
